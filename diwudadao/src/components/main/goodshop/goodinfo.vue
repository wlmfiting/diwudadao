<template>
  <div class="goods">
    ...
    <div class="info">
      <div class="price">
        <div>￥
          <span>{{goodsprice.rank_price}}</span>
        </div>
        <div>￥4730</div>
    ...
    <mt-popup v-model="flag1" position="bottom" class="installmentshow">
      <div class="ment">
        <h3>分期说明</h3>
        <div v-for="item in goodstogo1" class="ment1" @click="handelchange(item.installment_desc)">
          <p>{{item.installment_desc}}</p>
          <p>{{item.service_charge_desc}}</p>
        </div>
    ...
    <mt-popup v-model="flag2" position="bottom" class="installmentshow">
      <div class="ment">
        ...
        <div class="wrapper goodsWrapper" ref="goodsWrapper">
          <div class="content">
            <div v-for="item in goodAll" class="ment1">
              <p>{{item.name}}</p>
              <p class="mmm">{{item.desc}}</p>
            </div>
          </div>
        </div>
    ...
    <mt-popup v-model="flag3" position="bottom" class="installmentshow">
      <div class="ment">
        ...
        <div class="wrapper goodsWrapper" ref="canshuWrapper">
          <div class="content">
            <div v-for="item in goodsparaarr" class="ment1">
              <p></p>
              <p>{{item}}</p>
            </div>
          </div>
      ...
      <div class="ment ment1">
        ...
        <div class="model">
          <div class="model1">
            ...
            <ul class="modelul">
              <li
                v-for="(li,index) in goodsto0.attr_val"
                :class="index==colorIndex?'modelli':''"
                @click="handelgetcolorIndex(index,li.attr_value)"
              >{{li.attr_value}}</li>
            </ul>
          </div>
          <div class="model1">
            ...
            <ul class="modelul">
              <li
                v-for="(li,index) in goodsto1.attr_val"
                :class="index==sizeIndex?'modelli':''"
                @click="handelgetsizeIndex(index,li.attr_value)"
              >{{li.attr_value}}</li>
            </ul>
          </div>
          <h3>数量</h3>
          <div class="readu">
            <p @click="handelnumread()">-</p>
            <input type="text" v-model="num">
            <p @click="handelnumadd()">+</p>
          </div>
        </div>
...
.info > .price > div:nth-of-type(1) {
  color: #9b885f;
}
.info > .price > div:nth-of-type(1) > span {
  color: #9b885f;
  font-size: 0.45rem;
}
.info > .price > div:nth-of-type(2) {
  color: #333;
  text-decoration: line-through;
  padding: 0.15rem 0.2rem 0 0.2rem;
}
.info > .price > div:nth-of-type(3) {
  color: #333;
  border: 1px solid #333;
  padding: 0.03rem 0.2rem;
  font-size: 0.24rem/1 "微软雅黑";
  height: 0.34rem;
  margin-top: 0.1rem;
}
.shui {
...
.installmentshow > .ment {
  width: 100%;
  background: #fff;
  height: 100%;
  position: relative;
}
.installmentshow > .ment > h3 {
  width: 100%;
  text-align: center;
  padding: 0.3rem;
...
.installmentshow > .ment .ment1 {
  width: 100%;
  margin: 0 0.4rem;
  padding: 0.2rem 0;
  border-bottom: 2px solid #f2f2f2;
}
.installmentshow > .ment .ment1 > p:nth-of-type(2) {
  color: #f2f2f2;
  font-size: 0.24rem;
  color: #999;
}
.installmentshow > .ment .foot {
  width: 100%;
  height: 1rem;
  line-height: 1rem;
  font-size: 0.35rem;
...
.articlefont > p:nth-of-type(2) {
  font-size: 0.26rem;
  color: #9b885f;
}
.articlefont > p:nth-of-type(3) {
  color: #999;
}

.installmentshow > .ment1 .model {
  padding: 0.2rem;
  background: #fafafa;
  overflow: hidden;
}
